"""Canonical upstream prompt/tool material for the anonymous Zen free tier.

The free-tier gate matches requests against genuine opencode traffic
(bisected live). On the anonymous responses leg the proxy therefore
mirrors genuine shapes:

- no client tools: TITLE_PREFIX lead (title-task shape);
- client tools present: META_TEXT lead (agent-turn shape) + the genuine
  tool set below with client tools appended (superset verified live).

Keyed operators keep exact fidelity (none of this applies). Sources:
https://github.com/sst/opencode (dev)
- packages/opencode/src/agent/prompt/title.txt (TITLE_PREFIX)
- packages/opencode/src/session/prompt/meta.txt (META_TEXT,
  {{MODEL_NAME}} substituted per model)
Tracked by llms/proxy/zen_fingerprint.py.
"""

from __future__ import annotations

TITLE_SOURCE_URL = "https://raw.githubusercontent.com/sst/opencode/dev/packages/opencode/src/agent/prompt/title.txt"
META_SOURCE_URL = "https://raw.githubusercontent.com/sst/opencode/dev/packages/opencode/src/session/prompt/meta.txt"

# First 1500 chars of the title prompt + client text after SEAM passes the
# gate (verified live); shorter leads do not.
SEAM = "\n\n[Session task]\n"
...
META_TEXT = """You are OpenCode, a coding agent that helps users with software engineering tasks. You are powered by {{MODEL_NAME}}, a large language model trained by Meta MSL.
